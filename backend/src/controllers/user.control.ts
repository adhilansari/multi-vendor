...
import { isAuthenticated, isAdmin } from "../middleWares/auth.mid";

const router = Router();

// create user
router.post("/create-user", catchAsyncErrors(async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.post(
  ...
  catchAsyncErrors(async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!isPasswordValid) {
        res.send("Please provide the correct information")
...
router.put(
  ...
  catchAsyncErrors(async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (sameTypeAddress) {
        res.send(`${req.body.addressType} address already exists`)
...
router.delete(
  ...
  catchAsyncErrors(async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!user) {
        res.send("User is not available with this id")
